...
// Tanggal Surat (Bahasa Indonesia)
$bulan = [
...
// 3. Ambil Data Mahasiswa (Berdasarkan Jurusan & Lamaran ke Perusahaan tsb)
// NOTE: Jika tabel lamaran belum ada, query ini akan mengambil SEMUA mahasiswa dari jurusan tersebut.
// Jika tabel lamaran sudah ada, uncomment bagian JOIN lamaran.
$query_mhs = "SELECT m.nama_lengkap, m.nim, p.nama_prodi, u.no_hp
              FROM mahasiswa m
              JOIN users u ON m.id_user = u.id_user
              JOIN prodi p ON m.id_prodi = p.id_prodi
              WHERE p.id_jurusan = '$id_jurusan'
              -- AND EXISTS (SELECT 1 FROM lamaran l WHERE l.id_mahasiswa = m.id_mahasiswa AND l.id_perusahaan = '$id_perusahaan')
...
<head>
    ...
    <style>
        ...
        /* ISI SURAT */
        .nomor-area { margin-bottom: 20px; }
        .tujuan-area { margin-bottom: 20px; }
        ...
        /* HANYA MUNCUL DI LAYAR (NON-PRINT) */
        .no-print {
            position: fixed; top: 0; left: 0; width: 100%; background: #333; color: white;
            text-align: center; padding: 10px; font-family: sans-serif; z-index: 9999;
        }
...
<body onload="window.print()">
    ...
    <div class="nomor-area">
        <table style="border: none; width: 100%; margin: 0;">
            ...
            <tr style="border: none;">
                ...
                <td style="border: none;" colspan="2">: <b>Permohonan Magang</b></td>
            </tr>
        </table>
    </div>
    ...
    <p class="indent">
        Sebelumnya kami mendoakan Bapak/Ibu semoga sehat wal afiat dan sukses selalu dalam aktifitas sehari-hari. Amin.
    </p>

    <p class="indent">
        Politeknik Negeri Padang merupakan institusi pendidikan vocasional yang bertujuan untuk menciptakan lulusan profesional dan terampil dibidangnya, serta mampu mandiri dan bersaing ditingkat nasional maupun internasional. Untuk mewujudkan hal tersebut maka dari itu setiap mahasiswa tingkat akhir diwajibkan mengikuti Magang untuk menambah pengetahuan dan wawasan agar mereka betul-betul siap pakai.
    </p>

    <p class="indent">
        Sehubungan dengan itu, kami memohon kepada Bapak/Ibu untuk dapat menerima mahasiswa kami melaksanakan Magang untuk tahun ajaran <?= date('Y'); ?>/<?= date('Y')+1; ?> dari bulan <b><?= $tgl_mulai; ?> s/d <?= $tgl_selesai; ?></b> pada perusahaan/instansi yang Bapak/Ibu pimpin. Daftar nama mahasiswa kami sebagai berikut :
    </p>
    ...
    <table>
        ...
        <tbody>
            ...
            if(mysqli_num_rows($res_mhs) > 0) {
                ...
                echo "<tr><td colspan='5' style='text-align:center; padding: 20px;'>Belum ada data mahasiswa</td></tr>";
            }
            ?>
        </tbody>
    </table>
